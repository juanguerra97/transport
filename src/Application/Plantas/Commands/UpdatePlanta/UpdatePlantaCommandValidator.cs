...
public class UpdatePlantaCommandValidator : AbstractValidator<UpdatePlantaCommand>
{
    ...
    public UpdatePlantaCommandValidator(IApplicationDbContext context)
    {
        _context = context;

        RuleFor(v => v.PlantaId)
            .NotNull().WithMessage("El id de la planta es obligatorio.")
            .GreaterThan(0).WithMessage("El id de la planta debe ser un entero positivo");
